...
class WebSocketService {
  constructor() {
    ...
    this.live2dHeartbeatInterval = null;
  ...
  async connectLive2D(sessionId = null) {
    if (this.live2dWs && (this.live2dWs.readyState === WebSocket.CONNECTING || this.live2dWs.readyState === WebSocket.OPEN)) {
      console.log('[WebSocket] Live2D 연결이 이미 활성화되어 있습니다.');
      return Promise.resolve();
    }

    return new Promise((resolve, reject) => {
      try {
        const wsSessionId = sessionId || this.sessionId;
        const url = wsSessionId 
          ? `${this.baseUrl}/ws/live2d/${wsSessionId}`
          : `${this.baseUrl}/ws/live2d/anonymous`;
          
        console.log(`[WebSocket] Live2D 연결 시도: ${url}`);
        
        this.live2dWs = new WebSocket(url);
        
        this.live2dWs.onopen = (event) => {
          console.log('[WebSocket] Live2D 연결 성공');
          this.startLive2DHeartbeat();
          this.emit('live2dConnect', event);
          resolve();
        };
        ...
        this.live2dWs.onmessage = (event) => {
          try {
            ...
            console.error('[WebSocket] Live2D 메시지 파싱 실패:', error);
            this.emit('live2dError', { 
              type: 'message_parse_error', 
              data: event.data, 
              error: error.message 
            });
        ...
        this.live2dWs.onclose = (event) => {
          console.log('[WebSocket] Live2D 연결 종료:', event.code, event.reason);
          this.stopLive2DHeartbeat();
          this.emit('live2dDisconnect', event);
        };
        
      } catch (error) {
        console.error('[WebSocket] Live2D 연결 실패:', error);
        reject(error);
      }
    });
  }

  // Live2D WebSocket 연결 해제
  disconnectLive2D() {
    if (this.live2dWs) {
      console.log('[WebSocket] Live2D 연결 해제');
      this.stopLive2DHeartbeat();
      this.live2dWs.close(1000, 'Client disconnect');
      this.live2dWs = null;
    }
  }

  // Live2D 메시지 전송
  sendLive2DMessage(data) {
    if (!this.live2dWs || this.live2dWs.readyState !== WebSocket.OPEN) {
      console.warn('[WebSocket] Live2D 연결이 활성화되지 않음. 메시지 전송 실패:', data);
      return false;
    }

    try {
      const message = JSON.stringify(data);
      this.live2dWs.send(message);
      console.log('[WebSocket] Live2D 메시지 전송:', data);
      return true;
    } catch (error) {
      console.error('[WebSocket] Live2D 메시지 전송 실패:', error, data);
      return false;
    }
  }

  // Live2D 감정/모션 동기화 전송
  sendLive2DSync(emotion, motion = null, parameters = null) {
    return this.sendLive2DMessage({
      type: 'sync_state',
      emotion: emotion,
      motion: motion,
  ...
  // Live2D 감정 변경 전송
  sendLive2DEmotionChange(emotion, intensity = 0.8, duration = 0.5) {
    return this.sendLive2DMessage({
      type: 'emotion_change',
      emotion: emotion,
      intensity: intensity,
  ...
  // Live2D 모션 트리거 전송
  sendLive2DMotionTrigger(motion, priority = 1) {
    return this.sendLive2DMessage({
      type: 'motion_trigger',
      motion: motion,
      priority: priority,
  ...
  // Live2D 하트비트 시작
  startLive2DHeartbeat() {
    if (this.live2dHeartbeatInterval) {
      clearInterval(this.live2dHeartbeatInterval);
    }

    this.live2dHeartbeatInterval = setInterval(() => {
      if (this.live2dWs && this.live2dWs.readyState === WebSocket.OPEN) {
        this.sendLive2DMessage({
          type: 'heartbeat',
          timestamp: new Date().toISOString(),
          session_id: this.sessionId
        });
  ...
  // Live2D 연결 상태 확인
  isLive2DConnected() {
    return this.live2dWs && this.live2dWs.readyState === WebSocket.OPEN;
  }

  // 전체 연결 해제 (기존 메서드 확장)
  disconnect() {
    console.log('[WebSocket] 전체 연결 해제');
