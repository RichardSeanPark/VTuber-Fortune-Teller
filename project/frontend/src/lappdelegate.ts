...
export class LAppDelegate {
  ...
  private initializeSubdelegates(): void {
    ...
    // Use existing live2d-canvas instead of creating new one
    const existingCanvas = document.getElementById('live2d-canvas') as HTMLCanvasElement;
    if (existingCanvas) {
      this._canvases.pushBack(existingCanvas);
    } else {
      // Fallback: create canvas if not found
